...
function isLocked(repositoryPath: string) {
    const isLocked: any = _lockMap[repositoryPath];
...
export async function pullBranch(repository: Repository, localBranch: Branch): Promise<Oid | undefined> {
...
export async function pullMaster(repository: Repository): Promise<Oid | undefined> {
...
export function getToken(): string {
    const token: string | undefined = process.env.SDK_GEN_GITHUB_TOKEN;
    _validatePersonalAccessToken(token);

    return token!;
}

function _validatePersonalAccessToken(token: string | undefined): void {
    if (!token) {
        const text =
